...
import { ArchivePayloadArchivist } from '@xyo-network/node-core-model'
...
const getPayloads = async (archivist: ArchivePayloadArchivist, archive: string, hash: string, payloads: XyoPayload[], limit: number) => {
...
const handler: RequestHandler<PayloadChainPathParams, XyoPayload[]> = async (req, res) => {
  ...
  await getPayloads(await archivePayloadsArchivistFactory(archive), archive, hash, payloads, limitNumber)
  res.json(payloads)
}

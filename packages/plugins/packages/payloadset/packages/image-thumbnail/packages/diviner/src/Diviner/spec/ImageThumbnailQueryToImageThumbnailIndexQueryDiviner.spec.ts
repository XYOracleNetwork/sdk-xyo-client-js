...
describe('ImageThumbnailQueryToImageThumbnailIndexQueryDiviner', () => {
  ...
  describe('divine', () => {
    it.each(results)('transforms', async (imageThumbnailPayload) => {
      ...
      const sources = (await PayloadHasher.hashPairs([boundWitness, imageThumbnailPayload, timestampPayload])).map((source) => source[1])
      ...
      expect(index.sources).toEqual(sources)

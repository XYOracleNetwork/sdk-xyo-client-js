...
describe('/:hash', () => {
  describe('return format is', () => {
    ...
    it('a single BoundWitness matching the pointer criteria', async () => {
      ...
      const expected = BoundWitnessWrapper.parse(bw).jsonPayload()
  ...
  describe('with rules for', () => {
    describe('address', () => {
      ...
      describe('single address', () => {
        it.each([
          [accountA, () => BoundWitnessWrapper.parse(bws[0]).jsonPayload()],
          [accountB, () => BoundWitnessWrapper.parse(bws[1]).jsonPayload()],
      ...
      describe('multiple address rules', () => {
        describe('combined serially', () => {
          it('returns BoundWitness signed by both addresses', async () => {
            const expected = BoundWitnessWrapper.parse(bws[4]).jsonPayload()
    ...
    describe('schema', () => {
      ...
      describe('single schema', () => {
        it.each([
          [schemaA, () => BoundWitnessWrapper.parse(boundWitnesses[0]).jsonPayload()],
          [schemaB, () => BoundWitnessWrapper.parse(boundWitnesses[1]).jsonPayload()],
    ...
    describe('timestamp direction', () => {
      ...
      it('ascending', async () => {
        const expected = BoundWitnessWrapper.parse(assertEx(boundWitnesses.at(0))).jsonPayload()
        const pointerHash = await createPointer([[account.address]], [[expectedSchema]], 0, 'asc')
        const result = await getHash(pointerHash)
        expect(result).toEqual(expected)
      })
      it('descending', async () => {
        const expected = BoundWitnessWrapper.parse(assertEx(boundWitnesses.at(-1))).jsonPayload()

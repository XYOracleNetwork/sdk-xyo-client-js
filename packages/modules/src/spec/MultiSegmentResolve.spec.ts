...
describe('SimpleModuleResolverDeep', () => {
  test.skip('two archivists same name, different nodes', async () => {
    ...
    const resolvedModuleA = rootNode.resolve(nodeAName)
    expect(resolvedModuleA).toBe(moduleA.address)
    const resolvedModuleB = rootNode.resolve(nodeBName)
    expect(resolvedModuleB).toBe(moduleB.address)

    const resolvedModuleAMulti = rootNode.resolve(`${nodeAName}:${moduleAName}`)
    expect(resolvedModuleAMulti).toBe(moduleA.address)
    const resolvedModuleBMulti = rootNode.resolve(`${nodeBName}:${moduleBName}`)
    expect(resolvedModuleBMulti).toBe(moduleB.address)
  })
})

...
export const getArchivists = async (configs: XyoApiConfig[] = [getApiConfig()]): Promise<(ArchivistModule & AbstractModule)[]> => {
  const archive = getArchive()
  const archivists: (ArchivistModule & AbstractModule)[] = []

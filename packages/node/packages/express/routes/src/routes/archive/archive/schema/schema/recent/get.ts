...
const getRecentPayloadsOfSchemaForArchive = async (archivist: ArchivePayloadArchivist, schema: string, limit: number) => {
  const order = 'desc'
...
const handler: RequestHandler<ArchiveSchemaPayloadsRecentPathParams> = async (req, res) => {
  ...
  const schemas = (await getRecentPayloadsOfSchemaForArchive(await archivePayloadsArchivistFactory(archive), schema, limitNumber)) || []
  res.json(schemas)
}

...
import { CoinUserLocationsDivinerConfigSchema, CoinUserLocationsDivinerParams } from '@xyo-network/diviner-coin-user-locations-model'
...
> extends CoinUserLocationsDiviner<TParams> {
  static override configSchema = CoinUserLocationsDivinerConfigSchema
  ...
  async divine(payloads?: Payload[]): Promise<Payload[]> {
    assertEx(!payloads?.length, 'MemoryCoinUserLocationsDiviner.divine does not allow payloads to be sent')
    const archivists =

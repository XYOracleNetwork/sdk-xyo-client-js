...
export const augmentWithMetadata = async <T extends PayloadWithPartialMeta[] | BoundWitnessWithPartialMeta[]>(
...
): Promise<T extends PayloadWithMeta ? PayloadWithMeta[] : BoundWitnessWithMeta[]> => {
  ...
  )) as unknown as T extends PayloadWithPartialMeta ? PayloadWithMeta[] : BoundWitnessWithMeta[]
}

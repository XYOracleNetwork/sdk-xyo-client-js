...
describe.skip('WebsocketBridge', () => {
  const baseUrl = `${process.env.API_DOMAIN}` ?? 'ws://localhost:8080'

  console.log(`WebsocketBridge:baseUrl ${baseUrl}`)
  const cases = [
    ['/', `${baseUrl}`],
    /*['/node', `${baseUrl}/node`],*/
  ]

  it.each(cases)('WebsocketBridge: %s', async (_, url) => {
    const memNode = await MemoryNode.create({ account: Account.randomSync() })

    const bridge = await WebsocketBridge.create({
      account: Account.randomSync(),
      config: { name: 'TestBridge', schema: WebsocketBridgeConfigSchema, security: { allowAnonymous: true }, url },
    })
  ...
  it.each(cases)('WebsocketBridge - Nested: %s', async (_, url) => {
    ...
    const bridge = await WebsocketBridge.create({
      account: Account.randomSync(),
      config: { schema: WebsocketBridgeConfigSchema, security: { allowAnonymous: true }, url },
    })

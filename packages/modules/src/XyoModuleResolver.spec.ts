...
import { XyoArchivistGetQuerySchema, XyoMemoryArchivist } from '@xyo-network/archivist'
...
describe('XyoModuleResolver', () => {
  test('simple by address', async () => {
    const archivist: XyoModule = await XyoMemoryArchivist.create()
  ...
  test('simple by config', async () => {
    ...
    expect((await resolver.resolve({ config: [XyoMemoryArchivist.configSchema] })).length).toBe(1)
